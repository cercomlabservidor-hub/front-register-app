...
const EDUCATION_LEVELS = ['BÁSICO', 'SECUNDARIA', 'TÉCNICO', 'TECNOLÓGICO', 'SUPERIOR']
...
function App() {
  ...
  const collectStepErrors = (stepIndex) => {
    ...
    if (stepIndex === 0) {
      ...
      if (formData.documentType === 'OTRO' && !formData.otherDocumentType.trim()) {
    ...
    if (stepIndex === 1) {
      const requiredFields = {
        ...
        hasSpecialNeeds: 'Indica si cuentas con impedimentos físicos o necesidades especiales.',
      }
      ...
      if (formData.employmentStatus === 'EMPLEADO' && !formData.companyName.trim()) {
        nextErrors.companyName = 'Ingresa la empresa donde laboras.'
      }

      if (formData.hasSpecialNeeds === 'SI' && !formData.specialNeedsDescription.trim()) {
        nextErrors.specialNeedsDescription = 'Describe la necesidad o impedimento.'
      }
    }
    ...
    if (stepIndex === 2) {
      ...
      if (formData.selectedScheme === 'OTRO ESQUEMA' && !formData.otherSchemeName.trim()) {
    ...
    if (stepIndex === 3) {
      ...
      if (!formData.prerequisitesAccepted) {
        nextErrors.prerequisitesAccepted = 'Debes confirmar que conoces los prerrequisitos.'
      }
    ...
    if (stepIndex === 4) {
      const declarationsAccepted = Object.values(formData.declarations).every(Boolean)
      if (!declarationsAccepted) {
        nextErrors.declarations = 'Debes aceptar todas las declaraciones para finalizar.'
      }

      if (!signatureDataUrl) {
        nextErrors.signature = 'La firma es obligatoria para completar el registro.'
      }
    }
  ...
  const activeRequirements = REQUIREMENTS_BY_REQUEST[formData.requestType] ?? []
    ...
    return (
      <main className="app-shell">
        <section className="confirmation-card">
          ...
          <p>
            Capturamos la información del solicitante, las declaraciones y la firma digital en un
            flujo paso a paso.
          </p>
          ...
          <div className="confirmation-grid">
            ...
            <article className="summary-card">
              ...
              <div className="signature-preview">
                <img src={submittedRecord.signatureDataUrl} alt="Firma del solicitante" />
              </div>
              <p className="muted">
                Puedes conectar este payload a la base de datos o API corporativa cuando lo
                necesites.
              </p>
            </article>
          </div>
          ...
          <div className="actions-row actions-row--start">
            <button type="button" className="button button--primary" onClick={resetForm}>
    ...
    <main className="app-shell">
      <section className="hero-card">
        <div>
          ...
          <h1>Solicitud de evaluación y certificación</h1>
          <p>
            Diligencia el registro paso a paso. Te guiaremos desde tus datos personales hasta la
            firma final para que el proceso sea claro, ordenado e intuitivo.
          </p>
        </div>
        ...
        <aside className="steps-panel">
          ...
          <article className="info-card">
            ...
            <ul className="compact-list">
              <li>
                <strong>Solicitante:</strong> {formData.fullName || 'Sin diligenciar'}
              </li>
              <li>
                <strong>Documento:</strong>{' '}
                {formData.documentNumber ? `${selectedDocumentLabel} - ${formData.documentNumber}` : 'Sin diligenciar'}
              </li>
              <li>
                <strong>Solicitud:</strong> {formData.requestType || 'Sin seleccionar'}
              </li>
              <li>
                <strong>Esquema:</strong> {selectedSchemeLabel || 'Sin seleccionar'}
              </li>
            </ul>
          </article>
        </aside>
          ...
          {currentStep === 0 ? (
            <section className="section-stack">
              <header className="section-header">
                ...
                <p>
                  Empecemos con la información básica para identificar al candidato y sus datos de
                  contacto.
                </p>
              </header>

              <div className="form-grid form-grid--two">
                <Field label="Fecha de elaboración" error={errors.createdAt}>
                  <input
                    type="date"
              ...
              <div className="form-grid form-grid--three">
                ...
                <Field label="¿Cuál?" error={errors.otherDocumentType}>
                  <input
                    type="text"
                    value={formData.otherDocumentType}
                    onChange={(event) => updateField('otherDocumentType', event.target.value)}
              ...
              <div className="form-grid form-grid--two">
                ...
                <Field label="Lugar de nacimiento" error={errors.birthPlace}>
                  <input
                    type="text"
              ...
              <div className="form-grid form-grid--two">
                <Field label="Fecha de expedición" error={errors.expeditionDate}>
                  <input
                    type="date"
                    value={formData.expeditionDate}
                    onChange={(event) => updateField('expeditionDate', event.target.value)}
                  />
                </Field>

                <Field label="Lugar de expedición" error={errors.expeditionPlace}>
                  <input
                    type="text"
                    value={formData.expeditionPlace}
                    onChange={(event) => updateField('expeditionPlace', event.target.value)}
                    placeholder="Ciudad / municipio"
                  />
                </Field>
              </div>
              ...
              <div className="form-grid form-grid--three">
                <Field label="Lugar de residencia" error={errors.residencePlace}>
                  <input
                    type="text"
                    value={formData.residencePlace}
                    onChange={(event) => updateField('residencePlace', event.target.value)}
              ...
              <div className="form-grid form-grid--three">
                ...
                <Field label="RH" error={errors.rh}>
                  <input
                    type="text"
                    value={formData.rh}
                    onChange={(event) => updateField('rh', event.target.value.toUpperCase())}
                    placeholder="Ej. O+"
                  />
                </Field>
              ...
              <div className="form-grid form-grid--two">
                <Field label="Celular" error={errors.cellphone}>
                  <input
                    type="text"
              ...
              <div className="form-grid form-grid--three">
                <Field label="Nivel educativo" error={errors.educationLevel}>
                  <select
                ...
                <Field label="Título obtenido" error={errors.degreeTitle}>
                  <input
                    type="text"
                    value={formData.degreeTitle}
                    onChange={(event) => updateField('degreeTitle', event.target.value)}
                    placeholder="Ej. Técnico laboral"
                  />
                </Field>

                <Field label="Institución" error={errors.institutionName}>
                  <input
                    type="text"
          ...
          {currentStep === 1 ? (
            <section className="section-stack">
              <header className="section-header">
                ...
                <p>
                  Ahora cuéntanos sobre tu situación laboral actual y tu experiencia relacionada con
                  la competencia a certificar.
                </p>
              </header>
              ...
              <div className="form-grid form-grid--two">
                ...
                <Field label="Profesión, ocupación u oficio" error={errors.profession}>
                  <input
                    type="text"
              ...
              <div className="form-grid form-grid--two">
                ...
                >
                  <input
                    ...
                    onChange={(event) => updateField('experienceValue', event.target.value)}
                    placeholder="Ej. 12"
                  />
                </Field>

                <Field label="Unidad" error={errors.experienceUnit}>
                  <select
                    value={formData.experienceUnit}
                    onChange={(event) => updateField('experienceUnit', event.target.value)}
                  >
              ...
              >
                <textarea
                  ...
                  onChange={(event) => updateField('specialNeedsDescription', event.target.value)}
          ...
          {currentStep === 2 ? (
            <section className="section-stack">
              <header className="section-header">
                ...
                <p>
                  Selecciona el trámite que vas a realizar y el esquema o norma sectorial en la que
                  deseas certificarte.
                </p>
              </header>
              ...
              {errors.requestType ? <p className="section-error">{errors.requestType}</p> : null}

              <Field label="Esquema de certificación" error={errors.selectedScheme}>
                <select
                  value={formData.selectedScheme}
                  onChange={(event) => updateField('selectedScheme', event.target.value)}
                >
              ...
              <Field label="Otro esquema" error={errors.otherSchemeName}>
                <input
                  type="text"
                  value={formData.otherSchemeName}
                  onChange={(event) => updateField('otherSchemeName', event.target.value)}
          ...
          {currentStep === 3 ? (
            <section className="section-stack">
              <header className="section-header">
                ...
                <p>
                  Antes de firmar, confirma que conoces los soportes del proceso y define la
                  modalidad en la que tomarás la evaluación.
                </p>
              </header>
              ...
              {errors.modality ? <p className="section-error">{errors.modality}</p> : null}

              <article className="info-card">
                <h3>Checklist de soportes</h3>
                <ul className="check-list">
                  <li>Copia del documento de identidad.</li>
                  <li>Solicitud de evaluación y certificación diligenciada.</li>
              ...
              <div className="checkbox-stack">
                <label className="checkbox-card">
                  <input
                    ...
                    onChange={(event) =>
                      updateField('attachmentsAcknowledged', event.target.checked)
                    }
                  />
                  <span>
                    Confirmo que cuento con estos soportes o que los entregaré conforme al proceso.
                  </span>
                </label>
                {errors.attachmentsAcknowledged ? (
                  <p className="section-error">{errors.attachmentsAcknowledged}</p>
                ) : null}
                ...
                <label className="checkbox-card">
                  ...
                  />
                  <span>
                    Entiendo que el proceso puede suspenderse si no cumplo los prerrequisitos
                    mínimos para presentar las pruebas.
                  </span>
                </label>
                {errors.prerequisitesAccepted ? (
                  <p className="section-error">{errors.prerequisitesAccepted}</p>
                ) : null}
                ...
                <label className="checkbox-card">
                  <input
                    ...
                    onChange={(event) => updateField('dataPolicyAccepted', event.target.checked)}
                  />
                  <span>
                    Autorizo el tratamiento confidencial de mis datos personales según la política
                    de CERCOMLAB.
                  </span>
                </label>
          ...
          {currentStep === 4 ? (
            <section className="section-stack">
              <header className="section-header">
                ...
                <p>
                  Revisa las condiciones finales, acepta las declaraciones y firma en el recuadro
              ...
              {errors.declarations ? <p className="section-error">{errors.declarations}</p> : null}

              <article className="signature-card">
                <div className="signature-card__header">
                  <div>
                    <h3>Firma del solicitante</h3>
                    <p>Firma con mouse, lápiz digital o toque sobre la pantalla.</p>
                  </div>
                  <button
                    type="button"
                ...
                {errors.signature ? <p className="section-error">{errors.signature}</p> : null}
              </article>
              ...
              <article className="info-card">
                ...
                <ul className="compact-list">
                  <li>
                    <strong>Nombre:</strong> {formData.fullName || 'Sin diligenciar'}
                  </li>
                  <li>
                    <strong>Documento:</strong>{' '}
                  ...
                  <li>
                    <strong>Modalidad:</strong> {formData.modality || 'Sin seleccionar'}
                  </li>
                </ul>
              </article>
            </section>
